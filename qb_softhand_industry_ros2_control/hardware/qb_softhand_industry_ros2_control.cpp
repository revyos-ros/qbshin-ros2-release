...
#include "qb_softhand_industry_ros2_control/qb_softhand_industry_ros2_control.hpp"
...
hardware_interface::CallbackReturn QbSofthandIndustryHW::on_init(
  const hardware_interface::HardwareInfo & info)
{
  logger_ = std::make_unique<rclcpp::Logger>(
    rclcpp::get_logger("QbSofthandIndustryHW"));
...
hardware_interface::CallbackReturn QbSofthandIndustryHW::on_configure(
...
QbSofthandIndustryHW::export_state_interfaces()
{
  std::vector<hardware_interface::StateInterface> state_interfaces;

  // we have only one joint
...
QbSofthandIndustryHW::export_command_interfaces()
{
...
hardware_interface::CallbackReturn QbSofthandIndustryHW::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(*logger_, "Activating...");

  auto req = std::make_shared<qb_softhand_industry_srvs::srv::Trigger::Request>();
...
hardware_interface::CallbackReturn QbSofthandIndustryHW::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::return_type QbSofthandIndustryHW::read(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  this->getMeasurements(actual_pos, actual_vel, actual_effort, stamp);
  
  joint_velocities_[0]=(double)actual_vel/info_.transmissions.at(0).joints[0].mechanical_reduction;
  actuator_interfaces_.at(0).state_=(double)actual_pos;;
  ...
  joint_position_[0]= joint_interfaces_.at(0).state_;
  // log state data
  std::stringstream ss;
  ss << "State data:";
  for (const auto & transmission_info : info_.transmissions)
...
hardware_interface::return_type QbSofthandIndustryHW::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  float pos_cmd = actuator_interfaces_.at(0).command_;
  float vel_cmd = 3200;
  float curr_cmd = 450;
  this->setCommands(pos_cmd, vel_cmd, curr_cmd);
...
QbSofthandIndustryHW::InterfaceData::InterfaceData(const std::string & name)
: name_(name), command_(kNaN), state_(kNaN), transmission_passthrough_(kNaN)
{
}

void QbSofthandIndustryHW::initializeServicesAndWait(){
  get_measurements_client_ = node_->create_client<qb_softhand_industry_srvs::srv::GetMeasurements>("/qb_softhand_industry_communication_handler/get_measurements");
  set_command_client_ = node_->create_client<qb_softhand_industry_srvs::srv::SetCommand>("/qb_softhand_industry_communication_handler/set_command");
  set_commands_client_ = node_->create_client<qb_softhand_industry_srvs::srv::SetCommands>("/qb_softhand_industry_communication_handler/set_commands");
  activate_client_ = node_->create_client<qb_softhand_industry_srvs::srv::Trigger>("/qb_softhand_industry_communication_handler/activate_motors");
  deactivate_client_ = node_->create_client<qb_softhand_industry_srvs::srv::Trigger>("/qb_softhand_industry_communication_handler/deactivate_motors");
...
void QbSofthandIndustryHW::waitForSrvs(){
  ...
  set_command_client_->wait_for_service();
  set_commands_client_->wait_for_service();
  RCLCPP_INFO_STREAM(*logger_, "All necessary servers exist in [qb_softhand_industry_communication_handler]");
}

bool QbSofthandIndustryHW::setCommands(float &position, float &velocity, float &current){
  auto req = std::make_shared<qb_softhand_industry_srvs::srv::SetCommands::Request>();
  req->set_commands = true;
  req->position_command = position;
  req->velocity_command = velocity;
  req->current_command = current;
  req->max_repeats = max_repeats_;
...
bool QbSofthandIndustryHW::getMeasurements(float &position, float &velocity, float &current, rclcpp::Time &stamp) {
  auto req = std::make_shared<qb_softhand_industry_srvs::srv::GetMeasurements::Request>();
  req->get_position = true;
  req->get_current = true;
  req->get_velocity = true;
  req->max_repeats = max_repeats_;
    ...
    rclcpp::FutureReturnCode::SUCCESS)
  {
    ...
    RCLCPP_WARN_STREAM(*logger_, "Failed to get measurements.");
    return false;
  }
}
...
PLUGINLIB_EXPORT_CLASS(
  qb_softhand_industry_ros2_control::QbSofthandIndustryHW,
